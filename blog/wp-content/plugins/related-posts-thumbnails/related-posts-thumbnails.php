...
class RelatedPostsThumbnails {
	...
	function get_thumbnails( $show_top = false ) { // Retrieve Related Posts HTML for output
		...
		if ($relpoststh_output_style == 'list') {
			...
			$output .= '<div class="spliter"></div><div class="clearfix">';
		foreach( $posts as $post ) {
			...
			if ($relpoststh_output_style == 'list') {
				...
				$output .= '<a class="thumbnail" href="' . get_permalink( $post->ID ) . '">';
				$output .= "<div class=\"thumb-wrap\"><img src=\"{$url}\"></div>";
				$output .= "<h5>{$title}{$excerpt}</h5>";
				$output .= '</a>';
			}
