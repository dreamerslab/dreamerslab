...
// base uri
function dreamerslab_uri($controller='') {
  global $q_config;
  $base_uri = 'http://dreamerslab.com/';
  return $base_uri.$q_config['language'].'/'.$controller;
}

// blog uri
function dreamerslab_lang_short() {
  global $q_config;
  return $q_config['language'];
}

// blog uri
function dreamerslab_blog() {
  global $q_config;
  return "http://dreamerslab.com/blog/{$q_config['language']}/";
}

// lang switcher
function dreamerslab_lang() {
  global $q_config;
  $langs = qtrans_getSortedLanguages();
  $amount_of_lang = count($langs);
  $i = 0;

	if(is_404()) $url = get_option('home'); else $url = '';

  echo "<div id=\"lang\" lang=\"{$q_config['language']}\">";

  foreach( $langs as $language ){
    $i ++;

    $class = $language == 'tw' ? 'chinese' : '';
    if( $language == $q_config['language']){
      echo "<span class=\"{$class}\">{$q_config['language_name'][$q_config['language']]}</span>";
    }else{
      echo "<a class=\"{$class}\" href=\"".qtrans_convertURL($url, $language).'">';
...
// nav item
function dreamerslab_nav($item) {
  global $q_config;
  if( $q_config['language'] == 'tw' ){
